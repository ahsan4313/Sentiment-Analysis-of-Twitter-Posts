...
'''
    ...
    #start getResultHTML
    def getResultHTML(self, keyword, label,text,tweeti, time, pos_count, neg_count, neut_count, checked):
...
<body>
    ...
    <div class="yui3-u" id="bd">        
        <form name="keyform" id="key-form" method="get" onSubmit="return checkEmpty(this);">
        <p><input type="text" value="" name="keyword" id="keyword"/>       
        </form>       
        <div id="results">            
'''
        if(time == 'today'):
        ...
        while(t<length):
            if(label[t]=='positive'):
                left += '<li> ' + tweeti[t] + '</li>'
                #left += '<li title="'+self.weekDates[t]+'">' + tweeti[t] + '</li>'
            elif(label[t]=='neutral'):
                middle += '<li>' + tweeti[t] + '</li>'
            elif(label[t] == 'negative'):
                right += '<li> '+ tweeti[t] + '</li>'
            t=t+1
        #for i in results:
            #res= results[i]
            #for j in res:
